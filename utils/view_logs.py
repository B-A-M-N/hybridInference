...
def show_recent_logs(conn, limit=10):
    ...
    if rows:
        headers = ["Request ID", "Time", "Model", "Provider", "P.Tokens", "C.Tokens", "Total", "Latency(ms)", "Status", "Temp"]
        table_data = []
        for row in rows:
            table_data.append([
                row['req_id'],
...
def show_statistics(conn):
    ...
    if stats and stats['total_requests'] > 0:
        ...
        print(f"  • Successful: {stats['successful_requests']} ({stats['successful_requests']*100/stats['total_requests']:.1f}%)")
...
def show_sample_conversation(conn):
    ...
    if row:
        ...
        if row['temperature'] or row['top_p'] or row['max_tokens']:
            print(f"\n  Parameters:")
            if row['temperature']: print(f"  • Temperature: {row['temperature']}")
            if row['top_p']: print(f"  • Top-p: {row['top_p']}")
            if row['max_tokens']: print(f"  • Max Tokens: {row['max_tokens']}")
    else:
