...
SSH_HOST="${SSH_HOST:-}"
# Per-user run dir avoids predictable /tmp symlink races (CWE-59/CWE-377).
RUN_DIR="${RUN_DIR:-/tmp/h200_idle_proxy_$(id -u)_${LISTEN_PORT}}"
...
_list_containers() {
  # Prefer MODELS_CONFIG from the environment so paths with quotes/spaces are safe.
  python3 -c "import json, os; [print(v['container']) for v in json.load(open(os.environ['MODELS_CONFIG'])).values()]"
}
  ...
  start)
    ...
    if [[ -n "$SSH_HOST" && -n "$REMOTE_PORT" ]]; then
      ...
      for host in "${HOSTS[@]}"; do
        ...
        TUNNEL_PID=$!
        sleep 1
        # Always record one slot per host so status index stays aligned even
        # when a tunnel fails to start (placeholder "failed").
  ...
  stop)
    ...
    if [[ -f "$PID_FILE" ]]; then
      ...
      if kill -0 "$PID" 2>/dev/null; then
        ...
        kill "$PID"
        if [[ -f "$MODELS_JSON" ]]; then
          for ctr in $(_list_containers); do
    ...
    if [[ -n "$SSH_HOST" && -n "$REMOTE_PORT" ]]; then
      ...
      for host in "${HOSTS[@]}"; do
        ...
        if [[ $idx -lt ${#TUNNEL_PIDS[@]} ]]; then
          ...
          if [[ "$tpid" != "failed" ]] && kill -0 "$tpid" 2>/dev/null; then
            echo "Tunnel running (PID ${tpid}).  ${host}:${REMOTE_BIND}:${REMOTE_PORT} → localhost:${LISTEN_PORT}"
          else
            echo "Tunnel NOT running for ${host}:${REMOTE_BIND}:${REMOTE_PORT} → localhost:${LISTEN_PORT}."
          fi
        else
