    ...
    async def _restore_state(self, conn: sqlite3.Connection):
        ...
        for row in cursor:
            ...
            if tokens_val is not None and last_refill_val is not None:
                ...
                if window_start is not None and tokens_used is not None:
                    if now - float(window_start) < float(config.window_seconds):
                        bucket.tokens = max(0.0, min(float(config.capacity_tokens - tokens_used), float(bucket.capacity)))
...
@asynccontextmanager
async def create_rate_limiter(configs: List[RateLimitConfig], db_path: Optional[str] = None):
