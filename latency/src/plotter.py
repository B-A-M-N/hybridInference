...
def plot_tradeoff(results, model_name, local_hw, cloud_provider):
    ...
    for label, (latencies, cost) in results.items():
        ...
        plt.scatter(p99, cost, marker=markers['p99'], s=sizes['p99'], label=label, color=color, alpha=0.7)
...
def plot_pattern(hourly_total_requests):
    ...
    plt.boxplot(total_data, positions=hours, patch_artist=True,
    ...
    plt.title(f"Token Rate Distribution")
    plt.grid(True, linestyle='--', alpha=0.5)

    base_name = f"res/token_rate_original"
    # Save plot
    plt.savefig(f"{base_name}.pdf")
    plt.clf()
    
def plot_requests_by_hour(hourly_total_requests,  hourly_api_requests, model_name, local_hw, cloud_provider, label):
    if not os.path.exists("res"):
        os.makedirs("res")
    ...
    b1 = ax.boxplot(total_data, positions=positions_total, widths=0.35, patch_artist=True,
                    boxprops=dict(facecolor='skyblue'), medianprops=dict(color='black'))

    b2 = ax.boxplot(api_data, positions=positions_api, widths=0.35, patch_artist=True,
...
def plot_hourly_request_counts(hourly_total_requests, hourly_api_requests, model_name, local_hw, cloud_provider, label):
    if not os.path.exists("res"):
        os.makedirs("res")
    ...
    plt.plot(hours, count_api, marker='s', linestyle='--', label="API-Offloaded Requests", color="tab:red")

    plt.xlabel("Hour of Day")
    plt.ylabel("Tokens per second")
    plt.title(f"Token Rate\nModel={model_name}, GPU={local_hw}, Cloud={cloud_provider}, Config={label}")
    plt.xticks(hours)
    plt.grid(True, linestyle='--', alpha=0.5)
