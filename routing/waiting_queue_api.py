...
from typing import Optional, List, Dict, Set, Protocol
...
class OutsourcingRequestInfo:
    ...
    prefill_slo_seconds: Optional[float] = None  # TTFT deadline
    total_slo_seconds: Optional[float] = None     # End-to-end deadline
    ...
    def estimated_value(self) -> float:
        ...
        return (self.num_prompt_tokens * self.input_price_per_token + 
    ...
    @property
    def total_deadline(self) -> Optional[float]:
...
class WaitingQueueInterface(ABC):
    ...
    @abstractmethod
    def peek(self) -> Optional[OutsourcingRequestInfo]:
...
class OutsourcingEngine:
    ...
    def apply_outsourcing(self, decision: OutsourcingDecision) -> List[OutsourcingRequestInfo]:
        ...
        outsourced = self.waiting_queue.remove_requests(
            set(decision.requests_to_outsource)
        )
    ...
    def _check_slo_violations(
        ...
        for req in waiting_requests:
            ...
            req_flops = self.flop_calculator.compute_prefill_flops(
...
class SGLangWaitingQueueAdapter(WaitingQueueInterface):
    """
    Example adapter for SGLang's waiting queue.
    Replace with actual SGLang API calls.
    """
...
def example_usage():
    ...
    if decision.should_outsource:
        ...
        for req in outsourced_requests:
            print(f"  - Outsourcing request {req.request_id}: "
