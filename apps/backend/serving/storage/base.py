...
class OperationalStore(ABC):
    ...
    async def get_key_owner_for_audit(self, key_hash: str) -> Row | None:
        ...
        **Never for authentication.** Unlike the two lookups above, this one
        ...
        Returns ``user_id, role, key_status, key_expired, expires_in_sec,
        ...
        the same clock the auth lookups use, and ``expires_in_sec`` is measured
        against that same clock so a cache can stop serving that answer once
        the deadline passes — expiry being the one state change here that fires
        no write to invalidate on. ``expires_in_sec`` is ``None`` for a key
        with no deadline.
        """
...
class LogStore(ABC):
    ...
    async def get_user_detail_usage(
        ...
        *,
        ...
        (``None`` when they have none); ``ask_question_fraction`` is the share
        of the user's early-conversation requests (first few turns) whose
        ``tools`` offer an ask-the-user tool (``None`` when they have none).

        These three are computed only when ``include_activity_stats`` is set;
        otherwise those keys are ``None`` (the admin UI loads them on demand
        behind a button). The turn averages need full-history scans;
        ``ask_question_fraction`` scans only the early-conversation rows.
        """
    ...
    async def get_bulk_user_ask_question_fractions(
        ...
        "n_ask_requests"}`` over each user's early-conversation ``api_logs`` rows
        (first few turns). Users with no early-conversation requests are omitted.
        Used by the admin list endpoint to avoid N+1 queries.
        """
