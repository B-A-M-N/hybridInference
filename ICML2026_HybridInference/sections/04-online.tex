...
\end{equation}
where $L$ and $U$ are estimated lower and upper bounds on the request value (avoided API cost). A request is routed to $S_Q$ only if its value exceeds this threshold. This function is known to achieve optimal competitive ratios for the online knapsack problem.
...
\begin{equation}
    ...
    \min_{r \in \text{Queue}} \frac{v(r)}{p(r)} & \text{if saturated}
    \end{cases}
\end{equation}
...
We employ a \textbf{Learning-Augmented} approach:
\begin{enumerate}
    \item \textbf{Quantile Regression:} We train a lightweight model to predict the distribution of output tokens. Specifically, we predict the 10th percentile (P10) $\hat{L}^{(10)}_t$ and the median (P50) $\hat{L}^{(50)}_t$.
    \item \textbf{Conservative Value Estimation (LCB):} We use the Lower Confidence Bound (P10) to estimate the request value:
    \begin{equation}
        \hat{v}_t^{\text{LCB}} = p_{\text{in}} \cdot n_t^{\text{in}} + p_{\text{out}} \cdot \hat{L}^{(10)}_t
    \end{equation}
    This conservative estimate ensures we are "90\% confident" that the request is worth at least this much before spending quota on it.
\end{enumerate}
...
\begin{equation}
    G_t = \hat{v}_t^{\text{LCB}} - \psi(z_t) - \lambda_t \cdot \hat{p}_t
\end{equation}

where $\hat{p}_t$ is the estimated duration.
...
\begin{algorithmic}
   ...
   \STATE 4. Calculate Net Gain $G_t = \hat{v}^{\text{LCB}}_t - \psi(z_t) - \lambda_t \hat{p}_t$
