...
\end{equation}
This conservative estimate ensures that with high empirical coverage, the request is worth at least this much before spending quota on it. The intuition is simple: if even the pessimistic estimate of a request's value exceeds the opportunity cost threshold, we can confidently route it to the subscription.
...
This algorithm treats $S_Q$, $S_C$, and $S_A$ as three competing options, each with its own opportunity cost. When concurrency is not a bottleneck ($\lambda_t \approx 0$), the algorithm reduces to the optimal online knapsack policy for $S_Q$. When concurrency is tight, it naturally shifts to prioritizing short, high-value jobs via $S_C$. When both subscriptions are costly, requests spill to API.
