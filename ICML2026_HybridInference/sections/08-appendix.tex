...
where:
\begin{itemize}
    \item $z = k/Q$ is the fraction of quota consumed
    \item $L$ is the lower bound on request values (e.g., 5th percentile)
    \item $U$ is the upper bound on request values (e.g., 95th percentile)
\end{itemize}

\paragraph{Intuition.} The threshold starts at $L$ (low bar when quota is abundant) and grows exponentially to $U$ as quota depletes. This ensures:
\begin{itemize}
    \item Early requests with moderate value are accepted
    \item Late requests must have high value to justify consuming scarce quota
\end{itemize}
...
\begin{algorithmic}[1]
\STATE \textbf{Input:} Request $r_t$ with features $\mathbf{x}_t$, current quota usage $k_t$
\STATE \textbf{Parameters:} Quota $Q$, value bounds $[L, U]$, predictor $\mathcal{M}$
\STATE
...
\STATE
\STATE \textcolor{gray}{// Step 3: Compute threshold}
\STATE $z_t \gets k_t / Q$
\STATE $\theta_t \gets L \cdot (U/L)^{z_t}$
\STATE
\STATE \textcolor{gray}{// Step 4: Safeguard for suspicious predictions}
\IF{$\hat{v}_t^{\text{LCB}} < \epsilon$}
    \STATE $\hat{v}_t^{\text{LCB}} \gets \text{EMA\_estimate}(r_t)$ \textcolor{gray}{// Fallback}
\ENDIF
\STATE
\STATE \textcolor{gray}{// Step 5: Risk-aware routing decision}
\IF{$k_t < Q$ \AND $\hat{v}_t^{\text{LCB}} > \theta_t$}
    \STATE \textbf{return} Subscription
\ELSE
    \STATE \textbf{return} API
\ENDIF
\end{algorithmic}
\end{algorithm}
...
\paragraph{EMA Predictor Pseudocode.}
...
\paragraph{Histogram Predictor Pseudocode.}
...
RedNote & 54,599 & 84 & 8 & 2,341 & 3,892 \\
...
\begin{itemize}
    ...
    \item \textbf{RedNote:} Enterprise RAG workload with long contexts. Highest cost variance (CV > 2).
...
\paragraph{Per-Day Breakdown (ShareGPT).}
Table~\ref{tab:perday_sharegpt} shows daily results for the first 7 days of ShareGPT.

\begin{table}[h]
\caption{Per-Day Results on ShareGPT (Quota $Q=5000$)}
\label{tab:perday_sharegpt}
\begin{center}
\begin{tabular}{ccccccc}
\toprule
Day & Requests & Greedy & Optimal & PD-EMA & CR (Greedy) & CR (PD) \\
\midrule
1 & 6,823 & \$42.3 & \$38.1 & \$39.2 & 1.11 & 1.03 \\
2 & 7,156 & \$45.8 & \$40.2 & \$41.5 & 1.14 & 1.03 \\
3 & 6,542 & \$39.7 & \$35.8 & \$36.9 & 1.11 & 1.03 \\
4 & 7,891 & \$51.2 & \$44.3 & \$46.1 & 1.16 & 1.04 \\
5 & 6,234 & \$37.5 & \$33.9 & \$35.0 & 1.11 & 1.03 \\
6 & 5,987 & \$35.2 & \$32.1 & \$33.0 & 1.10 & 1.03 \\
7 & 7,445 & \$48.1 & \$42.0 & \$43.5 & 1.15 & 1.04 \\
\midrule
Avg & 6,868 & \$42.8 & \$38.1 & \$39.3 & 1.12 & 1.03 \\
\bottomrule
\end{tabular}
\end{center}
\end{table}

\paragraph{Multi-Model Results (FreeInference).}
FreeInference contains requests for 12 different models with varying API costs.

\begin{table}[h]
\caption{Per-Model Breakdown on FreeInference}
\label{tab:permodel_freeinference}
\begin{center}
\begin{tabular}{lcccc}
\toprule
Model & Requests & Avg Cost & Greedy Savings & Optimal Savings \\
\midrule
GPT-4 & 12,345 & \$0.85 & 71\% & 89\% \\
Claude-3 & 8,721 & \$0.62 & 68\% & 85\% \\
Llama-70B & 45,632 & \$0.23 & 58\% & 76\% \\
Mistral-7B & 89,234 & \$0.08 & 42\% & 61\% \\
\midrule
Overall & 371,214 & \$0.31 & 63\% & 82\% \\
\bottomrule
\end{tabular}
\end{center}
\end{table}

\textbf{Finding:} The gap between Greedy and Optimal is larger for expensive models (GPT-4: 18\% gap) than cheap models (Mistral-7B: 19\% gap). This is because the opportunity cost of wasting quota is higher when the avoided API cost is large.

\paragraph{Output Length Distribution.}
Table~\ref{tab:output_stats} shows the output length statistics for each dataset, explaining why RedNote has the highest cost variance.

\begin{table}[h]
\caption{Output Length Statistics by Dataset}
\label{tab:output_stats}
\begin{center}
\begin{tabular}{lccccc}
\toprule
Dataset & P10 & P50 & P90 & Mean & CV \\
\midrule
ShareGPT & 45 & 312 & 1,024 & 423 & 1.2 \\
FreeInference & 89 & 756 & 3,521 & 1,247 & 1.8 \\
RedNote & 234 & 2,145 & 12,456 & 3,892 & 2.4 \\
BurstGPT & 67 & 389 & 1,234 & 512 & 1.1 \\
\bottomrule
\end{tabular}
\end{center}
\end{table}

CV = Coefficient of Variation (std/mean). Higher CV indicates more opportunity for optimal routing to improve over greedy.
