...
\textbf{Empirical results.} On our latency benchmark (Llama-3.3-70B, 6,076 requests, SLO = 2s P99):
\begin{center}
\begin{tabular}{lcccc}
\toprule
Strategy & P99 (ms) & SLO Viol. & Cost (no-cancel) & Cost (cancel) \\
\midrule
OpenRouter Auto & 7,753 & 23.7\% & \$1.49 & -- \\
Smart Hedge (Ours) & 1,459 & 0.7\% & \$1.06 & \$0.91 \\
\bottomrule
\end{tabular}
\end{center}

Our smart hedging achieves 5.3$\times$ lower P99 latency and 33$\times$ fewer SLO violations. Without cancellation, cost savings are 29\%; with cancellation, savings increase to 39\%. The hedge rate is 15.7\% (953/6,076 requests), meaning the cancellation benefit is $(c_{\text{primary}} + c_{\text{backup}} - c_{\text{winner}}) \times 0.157 \approx 14\%$ additional savings.
...
Table~\ref{tab:output_stats} shows the output length statistics for each dataset, explaining why Enterprise has the highest cost variance.
...
Enterprise & 234 & 2,145 & 12,456 & 3,892 & 2.4 \\
