...
concurrency-limited subscription \(S_C\) alongside a fixed-window quota subscription
...
As in Stage~1, real-world subscriptions only cover subsets of models. This can be
captured via a compatibility predicate (or plan index) that restricts which requests
may be routed to \(S_Q\) and/or \(S_C\); for clarity we present a single \(S_Q\) and a
single \(S_C\).
...
Providers:
\begin{itemize}
  \item Fixed-window quota subscription \(S_Q\): at most \(Q_{\text{win}}\) requests per window of length \(W\) (aligned at \(t_0\)); zero marginal cost. Define
        \[
          \operatorname{win}(t) = \left\lfloor \frac{t - t_0}{W} \right\rfloor.
        \]
        A daily quota is the special case \(W=24\) hours with \(t_0\) aligned to midnight (in a chosen timezone).
...
\begin{itemize}
  ...
  \item \(S_Q\) is modeled purely as a per-window counter (concurrency is not the bottleneck).
...
Subject to:
\begin{align}
  &\text{(Quota)} && \sum_{i: \operatorname{win}(a_i)=k} x_i^Q \le Q_{\text{win}}, \quad \forall k, \\
...
\end{itemize}
If a request $i$ has no hard deadline, we set $d_i$ to a sufficiently large value (e.g., trace end time). If $\alpha_i > \beta_i$, the request cannot be scheduled on $S_C$.
...
\begin{align}
    \text{Minimize} \quad & \sum_{i \in \mathcal{R}} x_i^A c_i^{\text{API}} \label{eq:obj} \\
